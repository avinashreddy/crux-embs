...
public class EmbsFileProcessor implements QueueHandler {

    private static final Logger log = NotifyingLogger2.getLogger(EmbsFileProcessor.class, Level.ERROR);

    private static final String WORKFLOW_URI = "workflow://workflows/crux/embs/processFile";

    public static final String MESSAGE_CONTENT_TYPE = "text/vnd.rapture.ftp.embs.file";
    ...
    public boolean handleMessage(String tag, String routing, String contentType, RapturePipelineTask task) {
        log.info(format("Processing FileProcessingRequest message [%s]", new String(task.getContent())));
        try {
